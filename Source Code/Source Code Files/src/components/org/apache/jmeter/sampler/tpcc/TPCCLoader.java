...
/**
 * Contains the code for filling up the tables with the required data when testing MySQL database
 * @author naman
 *
 */
...
public class TPCCLoader
{
	...
	public TPCCLoader() {
        ...
        numWarehouses = (int)Math.round(configWhseCount);
        if (numWarehouses == 0)
        {
         numWarehouses = 1;
        }
	 ...
	 * @param strTable name of the table to be truncated
	 */
	...
	/**
	 * Contains code for loading a warehouse
	...
	protected int loadWhse(int whseKount) {
		...
		try {
                        ...
                        final String dburl=TPCCSampler.DBURL+TPCCSampler.DB_NAME;
                        this.conn = DriverManager.getConnection(dburl,TPCCSampler.USERNAME,TPCCSampler.PASSWORD);
                        System.out.println("dburl3"+dburl);
                        this.conn.setAutoCommit(false);
                        Statement s1=null;
                        s1=this.conn.createStatement();
	 ...
	 * @param whseKount number of warehouses
	 * @param distWhseKount number of district in each warehouse
	 * @return
	 */
	 ...
	 * @param whseKount no of warehouses
	 * @param distWhseKount district per warehouse
	 * @param custDistKount customer per district
	 * @return
	 */
	...
	/**
	 * method for loading the order table
	 * @param whseKount number of warehouses
	...
	/**
	 * method which first truncates the tables and then calls the load method for loading each table
	 * @throws SQLException
	 */
	//@Override
