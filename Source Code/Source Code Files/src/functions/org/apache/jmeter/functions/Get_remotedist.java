 ...
 * @since 1.9
 */
public class Get_remotedist extends AbstractFunction {
    ...
    private static final String KEY = "__remote_distno"; //$NON-NLS-1$
    ...
    public synchronized String execute(SampleResult previousResult, Sampler currentSampler)
       ...
       int min = 1;
    		   //Integer.parseInt(minimum.execute().trim());
        int max = 10;
        		//Integer.parseInt(maximum.execute().trim());
...
//        long rand = min + (long) (Math.random() * (max - min + 1));
        int ran2=Counter_tpcc.perThreadInt2.get();
        
        
        int ran3=0;
        if (ran2<=85)
        {	
        	ran3=Counter_tpcc.perThreadInt1.get();;
        }
        else
        {
        	int max2=1;
